...
| 34 | React 19, TypeScript 5 strict, Vite 7; CI pins Node 22 LTS (≥ 22.19). | Current stable releases; Vite 7 needs Node ≥ 20.19 and jsdom 30 (unit tests) needs undici 8, which needs Node 22.19. | accepted |
...
| 184 | Every project file is normalised on load: each record is coerced against the shape of a fresh record, references that point nowhere are dropped, and a record keyed `__proto__`/`constructor`/`prototype` is ignored. A schema version that is not a whole number is refused as invalid. | A hand-edited or third-party file used to pass migration and then crash the app, which the recovery screen exists to prevent; an id from a file must never be able to change an object's prototype. | decided |
| 185 | The page declares a content security policy in the document head: everything same-origin, no inline scripts, `data:`/`blob:` images for the PNG export and print preview, and no framing. | The app makes no network requests at all, so the policy can be strict; it is defence in depth for the two places that render generated HTML/SVG. | decided |
| 186 | Canvas dragging redraws once per animation frame from the latest pointer position; the exact positions live in refs, so what is committed on pointer up is unaffected. | A pointer reports several moves per frame and each redraw re-routed every connector in the tree. | decided |
| 187 | In the ancestor chart a repeated ancestor keeps the column it was first placed in and their partner joins them there, so a couple is never split; afterwards every column is swept so no two cards overlap, and the connector lines are drawn from the final positions. | Pedigree collapse means one person stands in two places at once; keeping the couple together and drawing the line to where the card actually is reads better than splitting them. | decided |
| 188 | Merging two people only tidies partnerships the merge itself touched. | A couple may legitimately be recorded as married twice; only the duplicate created by the merge should disappear. | decided |
| 189 | Spacing is two settings, across and down, each compact/normal/wide. A tree saved with the single earlier setting uses it for both axes, so nothing shifts on opening. | One control could not make a tree narrow without also making it flat; the two axes answer different questions (many siblings vs. many generations). | decided |
| 190 | Cards can be tinted by sex (male / female / diverse; unknown keeps the paper), off by default (it was on until 2026-09-18). The tint is a whisper — 1.1-1.2:1 against the paper — ink keeps at least 10:1 on it in both themes, and black-and-white printing drops it. | Colour reads faster than a marker at a glance, but it may never be the only carrier: the square / circle / diamond marker already states the same thing, which is what makes the tint safe to add. |
| 191 | What a card shows (places, occupation, group name) is a setting of the tree, not of the device, and a hidden line leaves its slot empty instead of shrinking the card. | The choice belongs to the drawing, so screen, print and the exports agree; a constant card height means changing it never re-arranges the tree. | decided |
| 192 | The PDF export is written in the app: a small PDF writer plus a converter for the subset of SVG the sheets use, fed the very SVG the SVG export writes. | The sheets are already one standalone SVG per page, so converting that keeps screen, paper, SVG and PDF the same drawing by construction, and it adds no dependency to an app that must run offline. | decided |
| 193 | Fonts are embedded as composite (Type0 / Identity-H) TrueType with a ToUnicode map, obtained by turning the shipped .woff chunks back into TrueType at export time. | A PDF cannot read WOFF or WOFF2, and WOFF2's glyph table is transformed rather than merely compressed; WOFF is plain zlib per table, which `DecompressionStream` undoes with no library. Identity-H removes the 255-character limit of a simple encoding, and ToUnicode is what makes the text searchable and copyable. | decided |
| 194 | East Asian trees get a PDF like any other: the app already ships Noto JP/KR/SC as ~120 unicode-range chunks per weight, and the export embeds only the chunks the tree's characters fall into. | "A CJK font is megabytes" is true of the whole font, not of the chunks the screen already uses; a few tens of kilobytes each is no different from the Latin case. | decided |
| 195 | The tint colours are the user's to choose. A pick is stored as picked — a full colour — and toned into a lightness band at drawing time: pale for the light palette, deep for the dark one. | One stored value then serves both themes and paper, the colour control shows a real colour rather than a near-white, and the contrast promise holds whatever is chosen instead of depending on taste. | decided |
| 196 | The band is verified rather than trusted: a test sweeps all 360 hues at five saturations and requires ink to keep 12:1 on a light tint, 8:1 on a dark one, slate 4.5:1 on either, and every tint to stay visible against its paper. | A free colour control can be given away safely only if no choice can produce an unreadable card. | decided |
| 197 | The WOFF copies of the East Asian chunks (18 MB in total) are copied from the fontsource packages by `prebuild` rather than kept in the repository, ship with the site, and are fetched on demand instead of precached — a runtime rule keeps each one after its first use. | The repository stays the size of its source, and a 35 MB install is not asked of someone whose tree is written in Latin script; the cost falls on the one export that needs them, once. | decided |
| 198 | An export whose fonts cannot be fetched fails with a message naming the problem, rather than writing a PDF with gaps where the names should be. | A file that looks finished but has holes in it is worse than no file. | decided |
| 199 | The tint starts off, and the square / circle / diamond marker is a separate switch that starts on. | Colour on every card is a strong default to impose on someone's family; the marker is the genealogical convention and the thing that survives black-and-white printing, so it is what a tree should have before anyone chooses otherwise. | decided |
| 200 | Turning the marker off while the tint is on is allowed, and the panel says plainly what it costs. | It is the reader's own tree and they may know their audience; what is not acceptable is taking the choice away silently or making it without telling them. | decided |
