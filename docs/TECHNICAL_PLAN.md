...
### 4.5 Layout (stage d)
1. Break cycles. 2. Split into connected components (persons + unions). 3. Per component: assign generations by longest-path from roots through union nodes (partners equalized to the same rank, adopted/step links included). 4. Couple blocks (partners adjacent, each partner on the side of their children); every block is owned by exactly one parent block (the family with more descendants when a couple has parents on both sides), siblings ordered by birth across all partnerships. 5. Coordinates by recursive subtree packing with per-row contours: children subtrees side by side with their own space, every partnership's junction centred over its children; root families placed side by side, in-law families above the person they married into (shifted by the least amount that keeps every row clear); unions become junction points between partners. 6. Components are packed left-to-right sorted by size, wrapping to a grid past a width threshold, with a gutter and a labelled boundary. People with `position: null` in an otherwise laid-out project are placed by running steps 3–5 on their component and translating only the null-position people to free space (existing positions are kept).
