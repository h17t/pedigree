...
export function layoutComponent(project: Project, members: string[], level: DetailLevel, adjIn?: Adjacency): LayoutResult {
  ...
  const compact = (r: number) => {
    ...
    for (let i = 1; i < flat.length; i++) {
      ...
      const minX = x.get(prev)! + COL + (blockStart.get(cur) ? BLOCK_GAP : 0);
  ...
  const centreOn = (ids: string[], targets: (id: string) => string[]) => {
    ...
    for (const b of [...new Set(ids.map((id) => blockOf.get(id)!))]) {
      ...
      const mean = t.reduce((a, id) => a + x.get(id)!, 0) / t.length;
      const width = (b.length - 1) * COL;
      const left = x.get(b[0]!)!;
      const desired = mean - width / 2;
  ...
  const centreRuns = (r: number) => {
    ...
    while (i < flat.length) {
      ...
      if (key !== '') {
        ...
        if (targets.length) {
          const mean = targets.reduce((a, id) => a + x.get(id)!, 0) / targets.length;
          const members = run.reduce((n, b) => n + b.length, 0);
          const span = (members - 1) * COL + (run.length - 1) * BLOCK_GAP;
  ...
  for (const id of members) positions.set(id, { x: Math.round((x.get(id) ?? 0) - minX), y: (rank.get(id) ?? 0) * rowH });
  return { positions, width: maxX - minX + card.width, height: maxRank * rowH + cardHeight(level), rows: maxRank + 1 };
}
