 ...
 */
export const Connectors = memo(function Connectors({ unions, background }: { unions: UnionGeometry[]; background?: string }) {
  const color = usePalette();
  const halo = background ?? color.ground;
  return (
    <g className="connectors" fill="none" strokeLinejoin="round" strokeLinecap="butt" aria-hidden="true">
      {unions.map((u) => (
        <g key={u.unionId}>
          {u.childLines.map((c) => (
            <g key={c.linkId}>
              <path d={c.d} stroke={halo} strokeWidth={6} />
              <path
                d={c.d}
          ...
          {u.partnerLine && (u.partnerLine.style === 'marriage' || u.partnerLine.style === 'divorced') && (
            <>
              ...
              <path d={u.partnerLine.d} stroke={halo} strokeWidth={4} />
            </>
          )}
          ...
          {u.partnerLine?.strike && (
            ...
            <g className="divorce-mark">
              <path d={`M${u.partnerLine.strike.x - 10} ${u.partnerLine.strike.y + 13} l8 -26 M${u.partnerLine.strike.x + 2} ${u.partnerLine.strike.y + 13} l8 -26`} stroke={halo} strokeWidth={7} strokeLinecap="round" />
