...
export type FontRange = 'latin' | 'latin-ext';
...
const UNICODE_RANGE: Record<FontRange, string> = { latin: unicodeRangeOf(LATIN_RANGES), 'latin-ext': unicodeRangeOf(LATIN_EXT_RANGES) };

export function fileFor(weight: FontWeight, range: FontRange): string {
  return `fonts/atkinson-hyperlegible-next-${range}-${weight}-normal.woff2`;
}

/** Which chunks a text needs, per weight used. */
export function chunksFor(text: string, weights: FontWeight[]): FontChunk[] {
  const needsLatin = inRanges(text, LATIN_RANGES);
  // The dagger (U+2020) sits in the Latin Extended chunk only.
  const needsExt = inRanges(text, LATIN_EXT_RANGES);
  const out: FontChunk[] = [];
  for (const weight of [...new Set(weights)].sort()) {
    if (needsLatin) out.push({ weight, range: 'latin', file: fileFor(weight, 'latin'), unicodeRange: UNICODE_RANGE.latin });
    if (needsExt) out.push({ weight, range: 'latin-ext', file: fileFor(weight, 'latin-ext'), unicodeRange: UNICODE_RANGE['latin-ext'] });
  }
  return out;
}
